...
export interface BeginBackupRequest { ownerID: string; localAudioID: string; byteCount: number; sha256: string; }
...
interface WorkerDependencies { authentication: WorkerAuthenticator; backups: BackupStore; multipart?: MultipartGateway; }

export function createWorker({ authentication, backups, multipart }: WorkerDependencies): ExportedHandler<Env> {
  return { async fetch(request): Promise<Response> {
    ...
    try {
      ...
      const match = suffix.match(/^\/([0-9a-f-]{36})(?:\/(multipart|complete|cancel)|\/parts\/(\d+)\/(url|confirm))?$/i);
      ...
      if (match[2] === "complete" && request.method === "POST") return await complete(backup, backups, multipart);
...
async function begin(request: Request, ownerID: string, backups: BackupStore): Promise<Response> { const body = await parseJSON(request); if (!isRecord(body) || typeof body.local_audio_id !== "string" || !uuidPattern.test(body.local_audio_id) || !validBytes(body.byte_count) || typeof body.sha256 !== "string" || !sha256Pattern.test(body.sha256)) throw new InvalidRequestError(); const created = await backups.begin({ ownerID, localAudioID: body.local_audio_id, byteCount: body.byte_count, sha256: body.sha256.toLowerCase() }); return Response.json(created, { status: 201 }); }
...
async function complete(backup: StoredBackup, backups: BackupStore, multipart: MultipartGateway): Promise<Response> { active(backup); const uploadID = uploadIDFor(backup); const parts = await backups.confirmedParts(backup.id); const count = Math.ceil(backup.byte_count / multipartPartSize); if (parts.length !== count || parts.some((part, index) => part.part_number !== index + 1 || part.byte_count !== expectedPartBytes(backup, part.part_number))) throw new InvalidRequestError(); const object = await multipart.complete(backup.object_key, uploadID, parts.map((part) => ({ partNumber: part.part_number, etag: part.etag, byteCount: part.byte_count }))); if (object.size !== backup.byte_count || object.sha256 !== backup.sha256) throw new Error("Completed object verification failed"); await backups.markBackedUp(backup); return Response.json({ id: backup.id, state: "backed_up" }); }
...
export function createDefaultWorker(supabaseURL: string, serviceRoleKey: string | undefined, multipart?: MultipartGateway): ExportedHandler<Env> { let authentication = authenticators.get(supabaseURL); if (!authentication) { authentication = new SupabaseJWTAuthenticator(supabaseURL); authenticators.set(supabaseURL, authentication); } return createWorker({ authentication, backups: serviceRoleKey ? new SupabaseBackupStore({ supabaseURL, serviceRoleKey }) : new UnconfiguredBackupStore(), multipart }); }
